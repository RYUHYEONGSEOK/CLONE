...
{
	if (!m_bCulling || m_bFillStart)	// 컬링에 걸린다면 return한다.
		return;
...
void CBot::RenderForWaterEffect(D3DXVECTOR4 vClipPlane, REFTYPE eRefType)
{
	XMFLOAT4X4 matWorld;
	...
	case OBJ_STATE_MOVE:	
		...
		if (!m_bInitState)
		{
			...
			m_dwIdx = CNaviMgr::GetInstance()->MoveOnNaviMesh(&m_pInfo->m_vPos
				, &(vDir * fTime), m_dwIdx, (eMapType)g_GameInfo.bMapType);
