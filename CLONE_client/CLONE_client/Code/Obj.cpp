...
CObj::CObj(CDevice * pDevice)
	...
	, m_pDepthShader(NULL)
	, m_pRefractionShader(NULL)
	, m_pReflectionShader(NULL)
	, m_pTexture(NULL)
	, m_pLightInfo(NULL)
	, m_fViewZ(0.f)
	, m_bCollision(false)
	, m_eRenderType(RENDERTYPE_END)
	, m_eObjRenderRefType(OBJ_RENDER_REF_END)
{
...
bool CObj::CheckRenderForRef(void)
{
	if (D3DXVec3Length(&(this->m_pInfo->m_vPos - D3DXVECTOR3(0.f, 0.f, 0.f))) < 60.f)
		return TRUE;
