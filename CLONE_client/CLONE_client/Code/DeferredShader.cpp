...
CDeferredShader::CDeferredShader(CDevice * pDevice)
	: CShader(pDevice)
{

}

CDeferredShader::CDeferredShader(const CDeferredShader & rhs)
...
CDeferredShader::~CDeferredShader(void)
{

}

HRESULT CDeferredShader::Ready_ShaderFile(const TCHAR * pFilePath)
{
	HRESULT hr = NULL;
...
void CDeferredShader::BeginShader(const int & iPassIdx)
{
}

void CDeferredShader::EndShader(void)
{
}

HRESULT CDeferredShader::SetShaderParameter(XMFLOAT4X4 worldMatrix, XMFLOAT4X4 viewMatrix, XMFLOAT4X4 projectionMatrix,
	CBuffer* pBuffer,
	...
	const WORD& wCntResource)
{
	...
	pDeviceContext->PSSetShaderResources(0, wCntResource, &(*ppResource));
	pDeviceContext->PSSetSamplers(0, 1, &m_pSampleState);

	return S_OK;
}

CDeferredShader * CDeferredShader::Create(CDevice * pDevice, const TCHAR * pFilePath)
{
	CDeferredShader*		pShader = new CDeferredShader(pDevice);
...
void CDeferredShader::Release(void)
{
}

CResource * CDeferredShader::Clone(void)
{
	++(*m_pRefCnt);
	return new CDeferredShader(*this);
}

void CDeferredShader::Update(void)
{
}
