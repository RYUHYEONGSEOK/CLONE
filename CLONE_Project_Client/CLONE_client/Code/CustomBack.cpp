...
void CCustomBack::FontUpdate(void)
{
		...
		if (bCheck[0] || bCheck[1])
		{
			if (bMouseOn[0] || bMouseOn[1])
			{
				if (!m_bButtonDown[0])
				{
					...
					CS_OptionChagne tPacket;
					tPacket.m_bIsHost = CServerMgr::GetInstance()->GetIsHost();
					tPacket.m_bDownMode = true;
					tPacket.m_bUpMode = true;
					tPacket.m_bDownRound = false;
					tPacket.m_bUpRound = false;
	...
	for (size_t i = 0; i <= CUS_FONT_BOTCNT; ++i)
	{
		...
		{
		case 0:
			if (g_GameInfo.bMode == true)
			{
				m_pFont[i]->m_wstrText = L"TEAM";
			}
			else if (g_GameInfo.bMode == false)
			{
				m_pFont[i]->m_wstrText = L"SINGLE";
			}
			break;
		case 1:
			m_pFont[i]->m_wstrText = to_wstring(g_GameInfo.iRoundCnt);
			break;
		case 2:
