...
void CServerMgr::ProcessPacket(char* _pPacket)
{
	...
	case SC_CONNECT_ROOM:
	{
		...
		g_GameInfo.iRoundCnt = pPacket->m_iRoundCount;
	...
	case SC_INIT_PLAYER:
	{
		...
		memcpy(m_bIsSoloOrTeam, pPacket->m_bIsSoloOrTeam, sizeof(bool) * MAX_USER);
	...
	case SC_CHANGE_STATE_BEAD:
	{
		...
		//수정필요
		//플레이어가 나일 경우에는 계속해서 구슬을 순회하면서 나의 구슬들을 조사해서 UI에 등록
	...
	case SC_CHECK_COLLISION:
	{
		...
		if (pPacket->m_iType == 1)
		{
			...
			if (pPacket->m_iMainID != m_iClientID)
			{
				...
				{
					((CStageUIFrame*)pStageUI)->AddKillLog(L"Enemy", L"Enemy");
				}
				else if (pPacket->m_iTargetID == m_iClientID)
				{
					((CStageUIFrame*)pStageUI)->AddKillLog(L"Enemy", L"Player");
				}
			}
			else if (pPacket->m_iMainID == m_iClientID)
			{
				if (pPacket->m_iTargetID != m_iClientID)
				{
					((CStageUIFrame*)pStageUI)->AddKillLog(L"Player", L"Enemy");
				}
				else if (pPacket->m_iTargetID == m_iClientID)
				{
		...
		else if (pPacket->m_iType == 2)
		{
			...
			if (pPacket->m_iMainID != m_iClientID)
			{
				((CStageUIFrame*)pStageUI)->AddKillLog(L"Enemy", wstrName);
			}
			else if(pPacket->m_iMainID == m_iClientID)
			{
	...
	case SC_GAME_STATE:
	{
		...
		{
			m_bIsWinOrLose = pPacket->m_bGameWinOrLose;
		}
		break;
	}
	default:
