...
CRenderTarget::CRenderTarget(CDevice* pDevice)
	...
	,m_pShadowStencilBuffer(NULL)
	,m_pShadowStencilView(NULL)

	,m_pRefractionTexture(NULL)
	,m_pRefractionResourceView(NULL)
	,m_pRefractionTargetView(NULL)
	,m_pRefractionDepthStencilBuffer(NULL)
	,m_pRefractionDepthStencilView(NULL)

	,m_pReflectionTexture(NULL)
	,m_pReflectionResourceView(NULL)
	,m_pReflectionTargetView(NULL)
	,m_pReflectionDepthStencilBuffer(NULL)
	,m_pReflectionDepthStencilView(NULL)
{	  
	...
	, DXGI_FORMAT Format, D3DXCOLOR Color)
{
	...
	depthBufferDesc.Width = iSizeX;
	depthBufferDesc.Height = iSizeY;
	...
	hr = m_pDevice->GetDevice()->CreateTexture2D(&depthBufferDesc, NULL, &m_pShadowStencilBuffer);
	FAILED_CHECK(hr);
	hr = m_pDevice->GetDevice()->CreateTexture2D(&depthBufferDesc, NULL, &m_pReflectionDepthStencilBuffer);
	FAILED_CHECK(hr);
	hr = m_pDevice->GetDevice()->CreateTexture2D(&depthBufferDesc, NULL, &m_pRefractionDepthStencilBuffer);
	FAILED_CHECK(hr);
	...
	hr = m_pDevice->GetDevice()->CreateDepthStencilView(m_pShadowStencilBuffer,
		&depthStencilViewDesc, &m_pShadowStencilView);
	FAILED_CHECK(hr);
	hr = m_pDevice->GetDevice()->CreateDepthStencilView(m_pReflectionDepthStencilBuffer, 
		&depthStencilViewDesc, &m_pReflectionDepthStencilView);
	FAILED_CHECK(hr);
	hr = m_pDevice->GetDevice()->CreateDepthStencilView(m_pRefractionDepthStencilBuffer,
		&depthStencilViewDesc, &m_pRefractionDepthStencilView);
	FAILED_CHECK(hr);
...
void CRenderTarget::Draw_RenderTarget(void)
{
	...
	m_matRTView._42 = -(WINCY - 50.f) + (WINCY >> 1);
	memcpy(&matView, m_matRTView, sizeof(float) * 16);
	((CTextureShader*)m_pShader)->SetShaderParameter(matWorld, matView, matProj, m_pTargetBuffer[4], NULL, m_pShadowResourceView);
...
{
	m_pDevice->GetDeviceContext()->OMSetRenderTargets(1, &m_pShadowTargetView, m_pShadowStencilView);

	m_pDevice->GetDeviceContext()->ClearRenderTargetView(m_pShadowTargetView, m_Color);
	m_pDevice->GetDeviceContext()->ClearDepthStencilView(m_pShadowStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);
}

void CRenderTarget::Set_Target_Refraction(void)
{
	m_pDevice->GetDeviceContext()->OMSetRenderTargets(1, &m_pRefractionTargetView, m_pReflectionDepthStencilView);

	m_pDevice->GetDeviceContext()->ClearRenderTargetView(m_pRefractionTargetView, m_Color);
	m_pDevice->GetDeviceContext()->ClearDepthStencilView(m_pReflectionDepthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);
}
