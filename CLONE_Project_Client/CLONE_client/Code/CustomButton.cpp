...
HRESULT CCustomButton::AddComponent(void)
{
	...
	pComponent = CResourceMgr::GetInstance()->CloneResource(CResourceMgr::RESOURCE_ATTRI_DYNAMIC, CResourceMgr::RESOURCE_TYPE_TEXTURE, L"Texture_CustomKJKPlaza");
	m_pCustomTexture[CUS_KJKPLAZA] = dynamic_cast<CTexture*>(pComponent);
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent.insert(make_pair(L"Texture", pComponent));
...
HRESULT CCustomButton::ButtonSetting(void)
{
	...
	//CUS_KJKPLAZA
	m_fSizeX[CUS_KJKPLAZA] = WINCX / 8.f;										m_fSizeY[CUS_KJKPLAZA] = WINCY / 4.5f;
	m_fX[CUS_KJKPLAZA] = WINCX / 2.f;											m_fY[CUS_KJKPLAZA] = WINCY / 2.f - m_fSizeY[CUS_KJKPLAZA] * 0.3f;
...
{
	CCustomButton*	pTerrain = new CCustomButton(pDevice);
