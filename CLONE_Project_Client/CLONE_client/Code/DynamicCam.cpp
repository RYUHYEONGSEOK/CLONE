	...
	if (CInput::GetInstance()->GetDIKeyState(DIK_P) & 0x80)
	{
		if (!m_bStaticCamKeyDown)
		{
			...
			if (!m_bStaticCam)
			{
				...
				m_dwNaviIdx = CNaviMgr::GetInstance()->GetNaviIndex(&m_vEye, m_dwNaviIdx);
			}
			else
				m_bStaticCam = FALSE;
		}
...
void CDynamicCamera::CamMove(void)
{
	...
	// 여기서 벽뚫는것 처리한다.
	m_dwNaviIdx = CNaviMgr::GetInstance()->MoveOnNaviMesh_Cam(&m_vEye, &m_vAt, &vCamDir, m_fTargetDistance, m_fCamY, m_dwNaviIdx);
}
