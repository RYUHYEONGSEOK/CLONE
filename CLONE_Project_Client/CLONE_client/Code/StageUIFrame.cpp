...
HRESULT CStageUIFrame::Init()
{
	...
	// Up Frame -------------------------------------------
	CStageUIFrame::m_fSizeX[UIFRAME_UPFRAME] = WINCX / 3.8f;
	CStageUIFrame::m_fSizeY[UIFRAME_UPFRAME] = WINCY / 14.f;

	CStageUIFrame::m_fX[UIFRAME_UPFRAME] = WINCX / 2.f - WINCX / 50.f;
...
int CStageUIFrame::Update()
{
	...
	if (*m_pPlayTime <= 0)
	{
		...
		if (m_fShowResultDelay <= 0.f)
		{
			...
			bool bIsWinOrLose = CServerMgr::GetInstance()->GetIsWinOrLose();
			if (bIsWinOrLose)
			{
				m_eStageState = STAGESTATE_VIC;
			}
			else if (!bIsWinOrLose)
			{
...
void CStageUIFrame::RenderBeadCheck(XMFLOAT4X4 matWorld, XMFLOAT4X4 matView, XMFLOAT4X4 matProj)
{
	if (m_pObjList != NULL)
	{
		...
		{
			if (reinterpret_cast<CBead*>(*iter_Beadlist)->GetOwnerID()
				== CServerMgr::GetInstance()->GetClientID())
			{
...
void CStageUIFrame::FontSetting(void)
{
	...
	for (size_t i = 0; i < FONT_END; ++i)
	{
		...
		case FONT_TIME:
			...
			m_pFont[i]->m_vPos = D3DXVECTOR2(WINCX / 2.f + WINCX / 35.f, WINCY / 9.5f);
			break;
		case FONT_PLAYERCNT:
			m_pFont[i]->m_wstrText = L"PLAYER";
			m_pFont[i]->m_vPos = D3DXVECTOR2(WINCX / 2.f + WINCX / 7.2f, WINCY / 11.f);
			break;
		case FONT_BOTCNT:
			m_pFont[i]->m_wstrText = L"BOT";
			m_pFont[i]->m_vPos = D3DXVECTOR2(WINCX / 2.f + WINCX / 4.8f, WINCY / 11.f);
			break;
		}
